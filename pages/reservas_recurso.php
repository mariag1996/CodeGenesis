...
try {
   ...
   $result = getDB()->query("SELECT id_recurso, nombre FROM recurso ORDER BY nombre");
    if ($result) {
    ...
    $msg = "Error al obtener recursos: " . htmlspecialchars($e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $conn = getDB();
    ...
    ");
    $check->bind_param("isss", $id_recurso, $fecha, $hora_inicio, $hora_fin);
    $check->execute();
    $resultado = $check->get_result()->fetch_assoc();


    if ($resultado['existe'] > 0) {
        $mensaje = "<p style='color:red; margin-top:10px;'>⚠️ El recurso ya está reservado para ese horario.</p>";
    } else {
        $stmt = $conn->prepare("
            INSERT INTO reserva_recurso (id_asignatura, id_recurso, id_docente, fecha, hora_inicio, hora_fin, estado)
            VALUES (?, ?, ?, ?, ?, ?, 'vigente')
        ");
        $stmt->bind_param("iiisss", $id_asignatura, $id_recurso, $id_docente, $fecha, $hora_inicio, $hora_fin);
...
<div class="casilla">
  ...
  <select name="id_recurso" required>
    ...
    <?php foreach ($recursos as $recurso): ?>
      <option value="<?= $recurso['id_recurso'] ?>"><?= htmlspecialchars($recurso['nombre']) ?></option>
    <?php endforeach; ?>
  </select>
 </div>
...
<div class="cont_titulo">
  <div class="contenedor__titulo-lista">
    <h2 class="titulo-lista">Lista de Reservas de Recursos</h2>
    </div>
    </div>
...
<table class="tabla_lista tabla" border="1" cellpadding="6" cellspacing="0">
    ...
    <tbody>
        ...
        <form method="POST" action="cancelar_reserva_recurso.php" style="display:inline;">
