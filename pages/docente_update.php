<!-- ......Este archivo se encarga de procesar el formulario de edición (docente_edit.php)........ -->
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: docentes_registro.php");
    exit;
}
...
$usuario = (int)($_POST['usuario'] ?? 0); // ID o cédula del docente (entero)
$nombre = trim($_POST['nombre'] ?? ''); // Nombre del docente
$apellido  = trim($_POST['apellido'] ?? ''); // Apellido del docente
...
try {
    //Se prepara una consulta SQL segura para actualizar los datos del docente.
    // Los signos de interrogación (?) son marcadores de posición.
    $stmt = getDB()->prepare("UPDATE docente SET nombre = ?, apellido = ? WHERE usuario = ?");
    //"ssi" indica los tipos de datos: string, string, integer.
...
// Si la ejecución fue correcta, se redirige al listado de docentes.
    header("Location: docentes_registro.php");
    exit;
