<!-- ......Este archivo se encarga de cancelar las reservas de aulas hechas por los docentes........ -->
<?php
// =====================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_reserva'])) {
    ...
    // Verificar que la reserva pertenece al docente
    $check = $conn->prepare("SELECT id_reserva FROM reserva_aula WHERE id_reserva = ? AND id_docente = ?");
    ...
    if ($res->num_rows === 1) {
        ...
        $update = $conn->prepare("UPDATE reserva_aula SET estado = 'cancelada' WHERE id_reserva = ?");
        $update->bind_param("i", $id_reserva);
        if ($update->execute()) {
        // Redirigir con mensaje de éxito
            header("Location: reservas_aula.php?msg=cancelada");
            exit();
        } else {
    ...
    header("Location: reservas_aula.php");
    exit();
}